...
class DiffusionCalibCacheLoader(BaseCalibCacheLoader):
    ...
    def _convert_layer_outputs(self, m: nn.Module, outputs: tp.Any) -> dict[str | int, tp.Any]:
        ...
        if isinstance(m, (FluxTransformerBlock, JointTransformerBlock)):
            ...
            # FluxSingleTransformerBlock.forward() can return:
            # - A single tensor (hidden_states) in standard diffusers
            # - A tuple (hidden_states, encoder_hidden_states) in some versions/configs
            # Handle both cases for compatibility.
            if isinstance(outputs, tuple):
                # Extract hidden_states from tuple (first element)
            ...
            return {0: hidden_states.detach().cpu()}
        else:
